/* ============================================================================
   maps.js — App "Mappe" (finestra w-fine di hub.php)
   ----------------------------------------------------------------------------
   Un percorso a tappe (Maturità → PCTO → Università → Estero) con camera SVG
   che fa pan & zoom, puck animato, tracciato che si disegna, elenco tappe
   cliccabile e controlli zoom/bussola. Toccando la mappa si avanza di tappa.
   Lo stile vive in assets/css/macos.css (.mv / .dir-* / .mv-ctrl / .mk ...).
   Usa i suoni e closeWin definiti in desktop.js.
   ============================================================================ */
(function () {
  ...
  /* Le tappe del viaggio: posizione sulla mappa, progresso sul tracciato e testi. */
  var stops = [
  ...
  var list = document.getElementById('nv-list'),
      elKk = document.getElementById('nv-kk'),
      elBody = document.getElementById('nv-body'),
      elStep = document.getElementById('nv-step'),
      elEta = document.getElementById('nv-eta'),
      elHint = document.getElementById('nv-hint');
  var i = 0, n = stops.length, j;

  /* Costruisce l'elenco delle tappe nel pannello "Indicazioni". */
  for (j = 0; j < n; j++) {
    var s = stops[j];
  ...
  /* Parametri della camera SVG (scala base, zoom, limiti di pan). */
  var sc = 1.16, zoom = 1, VW = 1000, VH = 640, TX = 600, TY = 320,
      minX = -300, maxX = 1300, minY = -260, maxY = 900;
  ...
  /* Chiusura della finestra dal pulsante ✕ del pannello indicazioni.
     closeWin (desktop.js) vuole l'elemento finestra, non il suo id. */
  var x = document.querySelector('#w-fine .dir-x');
  if (x) x.addEventListener('click', function (ev) {
    ev.stopPropagation();
    var w = document.getElementById('w-fine');
    if (w && typeof closeWin === 'function') closeWin(w);
  });
