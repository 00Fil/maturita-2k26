...
(function(){
  ...
  function syncStopMarkersToRoute(){stops.forEach((stop,i)=>{const pt=routePath.getPointAtLength(stop.p*totalLength);stop.x=pt.x;stop.y=pt.y;const ring=root.querySelector(`#${stop.id}`);if(ring){ring.setAttribute('cx',pt.x);ring.setAttribute('cy',pt.y);}const pin=Array.from(root.querySelectorAll('.poi-pin'))[i];if(pin){if(pin.classList.contains('poi-pin-3d')){pin.setAttribute('transform',`translate(${pt.x} ${pt.y})`);}else{pin.querySelectorAll('circle,ellipse').forEach(shape=>{shape.setAttribute('cx',pt.x);shape.setAttribute('cy',pt.y);});const t=pin.querySelector('text');if(t){t.setAttribute('x',pt.x);t.setAttribute('y',pt.y);}}}});}
