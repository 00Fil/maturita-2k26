...
/* Maps Navigator — smooth centered camera + inclined moving SVG grid */
(function(){
  ...
  const CAMERA_LERP=.105;
  const PHOTO_SLOW_SIGMA=.095, PHOTO_SLOW_STRENGTH=.56;
  const stops=[
  ...
  window.addEventListener('resize',()=>{centerCameraOnProgress(currentProgress,true);requestAnimationFrame(updateFloatingPhotoPositions);});
  ...
  function nearestProgress(x,y){let bestP=0,bestD=Infinity;for(let i=0;i<=9000;i++){const p=i/9000,pt=routePath.getPointAtLength(p*totalLength),d=Math.hypot(pt.x-x,pt.y-y);if(d<bestD){bestD=d;bestP=p;}}return bestP;}
  function resolveStopProgress(){stops.forEach(stop=>{stop.p=nearestProgress(stop.x,stop.y);});stops[0].p=0;syncStopMarkersToRoute();}
  function syncStopMarkersToRoute(){stops.forEach((stop,i)=>{const pt=routePath.getPointAtLength(stop.p*totalLength);stop.x=pt.x;stop.y=pt.y;const ring=root.querySelector(`#${stop.id}`);if(ring){ring.setAttribute('cx',pt.x);ring.setAttribute('cy',pt.y);}const pin=Array.from(root.querySelectorAll('.poi-pin'))[i];if(pin){const c=pin.querySelector('circle'),t=pin.querySelector('text');if(c){c.setAttribute('cx',pt.x);c.setAttribute('cy',pt.y);}if(t){t.setAttribute('x',pt.x);t.setAttribute('y',pt.y);}}});}
  ...
  function advance(){if(isRunning)return;if(finalOutPending){finishFinal();return;}if(finalPanoramaDone){resetNavigation();return;}if(currentStop>=stops.length-1){animateFinalStraight();return;}animateToStop(currentStop+1);try{sndOpen&&sndOpen();}catch(err){}}
  function animateToStop(targetStop){
    ...
    const segmentDuration=Math.max(5200,Math.min(13200,24500*delta*1.48)),t0=performance.now(),prev=new Set(triggeredCheckpoints);
    cancelAnimationFrame(raf);
    const tick=now=>{const raw=Math.min((now-t0)/segmentDuration,1),local=gaussianPhotoProgress(raw,fromProgress,toProgress);currentProgress=lerp(fromProgress,toProgress,local);renderMovingPoint(prev,1);if(raw<1)raf=requestAnimationFrame(tick);else{isRunning=false;currentStop=targetStop;currentProgress=toProgress;resetToStop(targetStop);}};
    raf=requestAnimationFrame(tick);
  }
  function animateFinalStraight(){
    if(isRunning)return;isRunning=true;finalOutPending=false;
    shell.classList.add('nav','maps-moving');shell.classList.remove('maps-paused','at-goal','maps-panorama-return','maps-final-wait');
    if(cue)cue.textContent='La strada continua…';updateSidebar(currentStop,'Tratto finale lineare','La camera segue, poi rallenta e si ferma gradualmente mentre il player continua dritto.');
    const fromProgress=currentProgress,toProgress=.78,duration=9200,releaseStart=.20,releaseEnd=.48,t0=performance.now(),prev=new Set(triggeredCheckpoints);
    cancelAnimationFrame(raf);
    const tick=now=>{const raw=Math.min((now-t0)/duration,1),local=gaussianPhotoProgress(raw,fromProgress,toProgress);currentProgress=lerp(fromProgress,toProgress,local);const followWeight=1-smootherstep((raw-releaseStart)/(releaseEnd-releaseStart));renderMovingPoint(prev,followWeight);if(raw<1)raf=requestAnimationFrame(tick);else{isRunning=false;finalOutPending=true;currentProgress=toProgress;shell.classList.remove('maps-moving');shell.classList.add('maps-final-wait');if(cue)cue.textContent='Tocca per confermare la visuale finale';updateSidebar(stops.length-1,'Visuale ferma','Il player continua dritto fuori inquadratura, senza mostrare la fine.');nextButtons.forEach(btn=>btn.textContent='Fine');}};
    raf=requestAnimationFrame(tick);
  }
  function finishFinal(){finalOutPending=false;finalPanoramaDone=true;shell.classList.remove('maps-final-wait','maps-moving','maps-paused','at-goal');shell.classList.add('maps-panorama-return');if(cue)cue.textContent='Tocca per resettare';updateSidebar(stops.length-1,'Percorso aperto','Dopo UniBS non c’è una destinazione: il tracciato continua.');nextButtons.forEach(btn=>btn.textContent='Reset');updateStepStates(stops.length-1);}
  function renderMovingPoint(previous,followWeight=1){const distance=currentProgress*totalLength,pt=routePath.getPointAtLength(distance),next=routePath.getPointAtLength(Math.min(distance+4,totalLength));moveCar(pt,getAngle(pt,next));updateProgress(distance);if(followWeight>0)centerCameraOnPoint(pt,false,followWeight);updatePhotoPinsByDistance(distance);updateCheckpoints(currentProgress,previous);updateFloatingPhotoPositions();}
  function centerCameraOnProgress(progress,instant){const pt=routePath.getPointAtLength(progress*totalLength);centerCameraOnPoint(pt,instant,1);}
  function centerCameraOnPoint(point,instant,weight=1){const rect=root.getBoundingClientRect(),aspect=rect.width/Math.max(1,rect.height),width=CAMERA_WIDTH,height=width/aspect,target=clampViewBox({x:point.x-width/2,y:point.y-height/2,width,height});if(instant){camera=target;}else{const k=CAMERA_LERP*Math.max(0,Math.min(1,weight));camera.x=lerp(camera.x,target.x,k);camera.y=lerp(camera.y,target.y,k);camera.width=lerp(camera.width,target.width,k);camera.height=lerp(camera.height,target.height,k);}applyCamera(camera);}
  function gaussianPhotoProgress(raw,fromProgress,toProgress){const start=Math.min(fromProgress,toProgress),end=Math.max(fromProgress,toProgress),span=Math.max(.0001,end-start),centers=photoPins.map(p=>(p.p-start)/span).filter(c=>c>.035&&c<.965);if(!centers.length)return smootherstep(raw);const steps=240;const speedAt=u=>{let slow=0;for(const c of centers)slow=Math.max(slow,Math.exp(-Math.pow((u-c)/PHOTO_SLOW_SIGMA,2)/2));return Math.max(.38,1-PHOTO_SLOW_STRENGTH*slow);};let total=0,partial=0,prev=0,prevSpeed=speedAt(0);for(let i=1;i<=steps;i++){const u=i/steps,sp=speedAt(u),area=(prevSpeed+sp)*(u-prev)/2;total+=area;if(raw>=u)partial+=area;else if(raw>prev){const local=(raw-prev)/(u-prev),interp=prevSpeed+(sp-prevSpeed)*local;partial+=(prevSpeed+interp)*(raw-prev)/2;}prev=u;prevSpeed=sp;}return Math.min(1,Math.max(0,partial/total));}
  function resetToStop(index){currentStop=index;currentProgress=stops[index].p;finalPanoramaDone=false;finalOutPending=false;const distance=currentProgress*totalLength,pt=routePath.getPointAtLength(distance),next=routePath.getPointAtLength(Math.min(distance+4,totalLength));moveCar(pt,getAngle(pt,next));updateProgress(distance);centerCameraOnPoint(pt,true,1);updateFloatingPhotoPositions();updateSidebar(index);updateStepStates(index);mapTilt.classList.add('navigation');if(index===0){shell.classList.remove('nav','maps-paused','maps-moving','at-goal','maps-panorama-return','maps-final-wait');if(cue)cue.textContent='Tocca la mappa per partire';}else{shell.classList.add('nav','maps-paused');shell.classList.remove('maps-moving','maps-panorama-return','maps-final-wait');triggerCheckpoint(stops[index].id);if(cue)cue.textContent=index>=stops.length-1?'Tocca per far uscire il player':'Tocca per proseguire';}nextButtons.forEach(btn=>btn.textContent=index>=stops.length-1?'Continua':(index===0?'Avvia':'Prosegui'));}
