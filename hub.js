...
/* Maps Navigator — centered follow camera + straight infinite final */
(function(){
  ...
  function nearestProgress(x,y){let bestP=0,bestD=Infinity;for(let i=0;i<=8000;i++){const p=i/8000,pt=routePath.getPointAtLength(p*totalLength),d=Math.hypot(pt.x-x,pt.y-y);if(d<bestD){bestD=d;bestP=p;}}return bestP;}
  function resolveStopProgress(){stops.forEach(stop=>{stop.p=nearestProgress(stop.x,stop.y);});stops[0].p=0;syncStopMarkersToRoute();}
  function syncStopMarkersToRoute(){stops.forEach((stop,i)=>{const pt=routePath.getPointAtLength(stop.p*totalLength);stop.x=pt.x;stop.y=pt.y;const ring=root.querySelector(`#${stop.id}`);if(ring){ring.setAttribute('cx',pt.x);ring.setAttribute('cy',pt.y);}const pins=Array.from(root.querySelectorAll('.poi-pin'));const pin=pins[i];if(pin){const circle=pin.querySelector('circle');const text=pin.querySelector('text');if(circle){circle.setAttribute('cx',pt.x);circle.setAttribute('cy',pt.y);}if(text){text.setAttribute('x',pt.x);text.setAttribute('y',pt.y);}}});}
  ...
  function animateToStop(targetStop){
    ...
    const segmentDuration=Math.max(4200,Math.min(11200,21000*delta*1.42)),t0=performance.now(),prev=new Set(triggeredCheckpoints);
    cancelAnimationFrame(raf);
    const tick=now=>{const raw=Math.min((now-t0)/segmentDuration,1),local=gaussianPhotoProgress(raw,fromProgress,toProgress);currentProgress=lerp(fromProgress,toProgress,local);renderMovingPoint(prev);if(raw<1)raf=requestAnimationFrame(tick);else{isRunning=false;currentStop=targetStop;currentProgress=toProgress;resetToStop(targetStop);}};
  ...
  function animateFinalStraight(){
    ...
    if(cue)cue.textContent='La strada continua…';updateSidebar(currentStop,'Tratto finale lineare','La camera segue per poco, poi resta ferma mentre il player continua dritto.');
    const fromProgress=currentProgress,toProgress=Math.min(.78,1),duration=8200,releaseAt=.24,t0=performance.now(),prev=new Set(triggeredCheckpoints);let releasedCamera=false;
    cancelAnimationFrame(raf);
    const tick=now=>{const raw=Math.min((now-t0)/duration,1),local=gaussianPhotoProgress(raw,fromProgress,toProgress);currentProgress=lerp(fromProgress,toProgress,local);const followCamera=raw<releaseAt;if(!followCamera&&!releasedCamera){releasedCamera=true;updateSidebar(stops.length-1,'Visuale ferma','Il player continua dritto fuori inquadratura, senza mostrare la fine.');}renderMovingPoint(prev,followCamera);if(raw<1)raf=requestAnimationFrame(tick);else{isRunning=false;finalOutPending=true;currentProgress=toProgress;shell.classList.remove('maps-moving');shell.classList.add('maps-final-wait');if(cue)cue.textContent='Tocca per confermare la visuale finale';updateSidebar(stops.length-1,'Player fuori oltre UniBS','La linea finale resta dritta e continua oltre la visuale.');nextButtons.forEach(btn=>btn.textContent='Fine');}};
    raf=requestAnimationFrame(tick);
  }
  ...
  function centerCameraOnPoint(point,instant){const rect=root.getBoundingClientRect(),aspect=rect.width/Math.max(1,rect.height),width=CAMERA_WIDTH,height=width/aspect,target=clampViewBox({x:point.x-width/2,y:point.y-height/2,width,height});if(instant){camera=target;}else{camera.x=lerp(camera.x,target.x,.18);camera.y=lerp(camera.y,target.y,.18);camera.width=lerp(camera.width,target.width,.18);camera.height=lerp(camera.height,target.height,.18);}applyCamera(camera);}
